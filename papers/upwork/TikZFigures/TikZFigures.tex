...
\begin{tikzpicture}[auto, node distance=2.7cm, >=latex']
  ...
  % Fig 3
  \tikzmath{\cy1 = (\ymax + \bh*2) / 2;}
  \draw (\s3,\ymax) .. controls (\s3+0.5,\cy1) .. (\s3+\cw1,\bh*2);
  \draw (\s3+\cw1,\bh*2) -- (\s3+\cw1,0) -- (\s3+\cw1+\width,0) -- (\s3+\cw1+\width,\bh*2);
  \draw (\s3+\cw1+\width,\bh*2) .. controls (\s3+\cw1+\width+1.5,\cy1) .. (\s3+\cw1*2+\width,\ymax);
  \draw[pattern={north east lines}] (\s3+\cw1,0) rectangle +(\width,\bh);

  % Fig 4
  \draw (\s4,\ymax) -- (\s4-\cw2,\bh*2) -- (\s4-\cw2,0) -- (\s4-\cw2+\width,0);
  \draw (\s4-\cw2+\width,0) -- (\s4-\cw2+\width,\bh*2) -- (\s4+\d*2,\ymax);
